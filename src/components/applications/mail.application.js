...
// Service  service_jh9r8tc
function Mail() {
	useEffect(() => {
		window.emailjs.init("TUDURXHMMGoQhpzqp");
	});
	...
	const sendFeedback = (templateId, variables) => {
		window.emailjs
			.send("service_jh9r8tc", templateId, variables)
			.then((res) => {
				var templateMeta = {
					from: "",
					subject: "",
			...
			.catch((err) => {
				console.error(
					"Oh well, message sending failed. Here some thoughts on the error that occurred:",
					err
				);
				var templateMeta = emailResponse.template;
			...
			<form onSubmit={handleSubmit}>
				...
				<div className="uk-margin form-input">
					<TextField
						...
						underlined
						disabled
						placeholder="Aenosh Rajora (aenoshrajora79@gmail.com)"
					/>
				</div>
				<div className="uk-margin form-input">
					<TextField
